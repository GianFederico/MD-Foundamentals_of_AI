:- module(explainuser_interaction_module, [askif/1, askifnot/1, start/0, askfor/1]).
:- dynamic(asked/2).

askif(Q) :- ask(Q,A), positive_answer(Q,A).

positive_answer(_,A) :- affirmative(A).
positive_answer(Qcode,A) :- \+(negative(A)), \+(affirmative(A)), write('Please answer yes or no.'), 
                            read(A2), retract(asked(Qcode,A)), asserta(asked(Qcode,A2)), affirmative(A2).
